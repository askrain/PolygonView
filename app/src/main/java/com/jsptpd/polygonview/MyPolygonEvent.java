package com.jsptpd.polygonview;
...
//        polyPaint.setFilterBitmap(true);
        ...
        Log.d("ss", "copy后的集合是"+dest1.toString());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
                ...
                for (int i = 0; i < pointList.size(); i++) {
                    ...
                    if (culDis < judgeDis && culDis != 0) {
                        if (i == (pointList.size() - 1)) {
                            ...
                            Log.d("ss", "linePointList中添加的坐标是：index= " + 1 + "------->" + "(" + linePointList.get(1).x + "," + linePointList.get(1).y + ")" + "index=" + 0 + "------->" + "(" + linePointList.get(0).x + "," + linePointList.get(0).y + ")");
                            ...
                            Log.d("ss", "linePointList中添加的坐标是：index= " + 0 + "------->" + "(" + linePointList.get(0).x + "," + linePointList.get(0).y + ")" + "index=" + 1 + "------->" + "(" + linePointList.get(1).x + "," + linePointList.get(1).y + ")");
                            index = i;//获取需要改变的点的下标
                            Log.d("ss", "index=  " + index);
                                /*再次做判断0和1*/
            ...
            case MotionEvent.ACTION_MOVE:
              /* 指导思想：从linePointList取出所有点遍历。最多三个点。要么两点x坐标相等，要么两点y坐标相等
                ...
                if (linePointList.size() > 0) {
                    ...
                    Log.d("ss", "从linePointList取出的点的坐标是：" + "(" + linePointList.get(0).x + "," + linePointList.get(0).y + ")" + "和  （" + linePointList.get(1).x + "," + linePointList.get(1).y + ")");
                             /*两个旧端点坐标  即起点坐标*/
            ...
            /* 两个新端点的坐标  给新坐标设置的初始值为旧坐标*/
                float newReferPoint1X = referPoint1X, newReferPoint2X = referPoint2X, newReferPoint1Y = referPoint1Y, newReferPoint2Y = referPoint2Y;
                if (dx > dy) {
                    ...
                    if (Math.min(referPoint1Y, referPoint2Y) < curY && curY < Math.max(referPoint1Y, referPoint2Y)) {
                        if (curX > referPoint1X) {
                            ...
                            if (index == -1) {
                                ...
                                pointList.get(pointList.size() - 1).x = (int) (pointList.get(pointList.size() - 1).x + dx);
                            } else {
                    ...
                    if (Math.min(referPoint1X, referPoint2X) < curX && curX < Math.max(referPoint1X, referPoint2X)) {
                        if (curY > referPoint1Y) {
                            ...
                            if (index == -1) {
                                ...
                                pointList.get(pointList.size() - 1).y = (int) (pointList.get(pointList.size() - 1).y + dy);
                            } else {
    ...
    /*海伦公式求点到直线的距离  原理：求出三点围成的三角形的面积除以底边（直线的长度）得到三角形的高（bug是理论上可能存在点在直线上的情况 实际情况是不存在按在直线上的情形）*/
    public float calDistanceByHelen(Point point, Point first, Point second) {
        ...
        /*面积*/
        float area = (float) Math.sqrt(perimeterHalf * (perimeterHalf - lineDis) * (perimeterHalf - dis1) * (perimeterHalf - dis2));
